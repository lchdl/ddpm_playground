...
def collect_train_data_CelebA():
    #
    train_xlsx = 'celeba_train.xlsx'
    val_xlsx = 'celeba_val.xlsx'
    test_xlsx = None
    #
    xlsx = '/data7/chenghaoliu/Codes/DIP/diffusion/diffusion/datasets/CelebA_cropped_128x128.xlsx'
    db = Database(db_keys=DDPM_Loader.db_keys, xlsx_file=xlsx)
    db.shuffle(9669)
    train_ratio = 0.9
    db_train, db_val = db.split([train_ratio, 1.0-train_ratio])
    db_train.export_xlsx(train_xlsx) if not file_exist(train_xlsx) else None
    db_val.export_xlsx(val_xlsx) if not file_exist(val_xlsx) else None
    return train_xlsx, val_xlsx, test_xlsx
...
if __name__ == '__main__':
    ...
    train_xlsx, val_xlsx, test_xlsx = collect_train_data_CelebA()
