...
from diffusion.utilities.file_ops import file_exist, join_path, mkdir
...
def hash_dict(d: dict, hlen=None) -> str:
    '''
    Hash a dictionary object `dict` into a string and truncate its length to `hlen`.
    Return the truncated string. If `hlen`=`None`, string will not be truncated.
    * Original implementation from:
    ...
    s = dhash.hexdigest()
    return s if hlen is None else s[:hlen]
